...
#define MAX  100
#define NULO -1
/* A constante NULO aqui faz o mesmo papel de NULL 
 * quando utilizamos ponteiros.
 */
 ...
 */
void
remova(int p, int *inicio, Celula v[], int *livre)
{
  int morta;
 ...
 */
void 
insira(int x, int *inicio, Celula v[], int *livre)
{
  int nova;
  int p;
 ...
 */
int
buscaRemove(int x, int *inicio, Celula v[], int *livre)
{
 ...
 */
void 
inicializaListaLivre(int *livre, int n, Celula v[])
{
  int i;
...
int 
extraiCelula(int *livre, Celula v[])
{
  int p = *livre;
...
void 
devolveCelula(int p, int *livre, Celula v[])
{
  v[p].prox = *livre;
  *livre = p;
}
